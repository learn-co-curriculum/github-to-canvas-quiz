# frozen_string_literal: true

RSpec.describe GithubToCanvasQuiz::Converter::Answer::MultipleAnswers do
  describe '.from_canvas' do
    it 'creates an instance of the class with the correct attributes' do
      answer = described_class.from_canvas({
        'id' => 3673,
        'text' => '',
        'html' => '<pre>function Card() {<br />&nbsp; return (<br />&nbsp; &nbsp; &lt;div&gt;<br />&nbsp; &nbsp; &nbsp; &lt;h1&gt;React&lt;/h1&gt;<br />&nbsp; &nbsp; &nbsp; &lt;p&gt;Component&lt;/p&gt;<br />&nbsp; &nbsp; &lt;/div&gt;<br />&nbsp; );<br />}</pre>',
        'comments' => '',
        'comments_html' => '',
        'weight' => 100.0
      })
      expect(answer).to have_attributes(
        title: 'Correct',
        text: '<pre>function Card() {<br />&nbsp; return (<br />&nbsp; &nbsp; &lt;div&gt;<br />&nbsp; &nbsp; &nbsp; &lt;h1&gt;React&lt;/h1&gt;<br />&nbsp; &nbsp; &nbsp; &lt;p&gt;Component&lt;/p&gt;<br />&nbsp; &nbsp; &lt;/div&gt;<br />&nbsp; );<br />}</pre>',
        comments: ''
      )
    end
  end

  describe 'instance methods' do
    let(:answer) do
      described_class.new(
        text: '<code>const</code>',
        comments: '<p>Correct!</code>',
        title: 'Correct'
      )
    end

    describe '#to_markdown' do
      it 'produces the correct markdown' do
        match = File.read('spec/fixtures/markdown/answer/multiple_answers_question.md')
        expect(answer.to_markdown.chomp).to eq(match.chomp)
      end
    end

    describe '#to_h' do
      it 'produces the correct hash' do
        expect(answer.to_h).to eq({
          'answer_html' => '<code>const</code>',
          'answer_weight' => 100,
          'answer_comment_html' => '<p>Correct!</code>'
        })
      end
    end
  end
end
